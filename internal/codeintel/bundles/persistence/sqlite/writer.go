...
func (w *sqliteWriter) WriteMeta(ctx context.Context, metaData types.MetaData) error {
	scheamVersionInserter := sqliteutil.NewBatchInserter(w.store, "schema_version", "version")
	if err := scheamVersionInserter.Insert(ctx, migrate.CurrentSchemaVersion); err != nil {
		return errors.Wrap(err, "scheamVersionInserter.Insert")
	}
	if err := scheamVersionInserter.Flush(ctx); err != nil {
		return errors.Wrap(err, "scheamVersionInserter.Flush")
	}

	metaInserter := sqliteutil.NewBatchInserter(w.store, "meta", "num_result_chunks")
	if err := metaInserter.Insert(ctx, metaData.NumResultChunks); err != nil {
		return errors.Wrap(err, "metaInserter.Insert")
	}
	if err := metaInserter.Flush(ctx); err != nil {
...
func (w *sqliteWriter) WriteDefinitions(ctx context.Context, monikerLocations []types.MonikerLocations) error {
	inserter := sqliteutil.NewBatchInserter(w.store, "definitions", "scheme", "identifier", "data")
...
func (w *sqliteWriter) WriteReferences(ctx context.Context, monikerLocations []types.MonikerLocations) error {
	inserter := sqliteutil.NewBatchInserter(w.store, `references`, "scheme", "identifier", "data")
	for _, ml := range monikerLocations {
		data, err := w.serializer.MarshalLocations(ml.Locations)
		if err != nil {
			return errors.Wrap(err, "serializer.MarshalLocations")
